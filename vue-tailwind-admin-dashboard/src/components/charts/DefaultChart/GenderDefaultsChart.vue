<template>
        <div class="max-w-full overflow-x-auto custom-scrollbar">
                <div id="genderDefaultsChart" class="-ml-5 min-w-[650px] xl:min-w-full pl-2">
...
async function fetchData() {
  const res = await fetch('http://zzh.hengtai119.cn/api/genderdefaults')
...
onMounted(async () => {
  ...
  // 整理数据格式，转换成 [{ gender: '男', defaulted0: count, defaulted1: count }, ...]
  const grouped = new Map()
  raw.forEach(d => {
    const g = genderMap[d.gender] || '未知'
    if (!grouped.has(g)) grouped.set(g, { gender: g, defaulted0: 0, defaulted1: 0 })
    const obj = grouped.get(g)
